...
const RESTART_SYNCH_DELAY = 30 // in seconds
...
const startSynchronizationLoop = asyncDebounce(synchronizationLoop, RESTART_SYNCH_DELAY * 1_000)
  ...
  renderNode: {
    ...
    [BLOCKS.EMBEDDED_ENTRY]: node => {
      ...
      if  (entry?.sys?.contentType?.sys?.id === 'textImageContainer') {
        const textImageContainer = new TextImageContainer(entry)
